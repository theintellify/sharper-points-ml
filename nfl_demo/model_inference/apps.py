...
# from model_inference.load_play_by_play import load_model

# from model_inference.models import play_by_play
import pandas as pd
from model_inference.utils import *
...
from model_inference.DATE_NUMBER_TO_WEEK_NUMBER_data import *
from datetime import datetime


DATE_NUMBER_TO_WEEK_NUMBER = {2024: DATE_NUMBER_TO_WEEK_NUMBER_2019,
                              2023: DATE_NUMBER_TO_WEEK_NUMBER_2018}

from datetime import datetime, timedelta


def get_nfl_week(start_date_str):
    # Define the NFL start date
...
# Example usage
# start_date = "2024-09-05"  # Example start date of the 2024 NFL season
# current_week = get_nfl_week(start_date)
...
class Game:
    ...
    def process_row(self, row, next_row=None):
        ...
        def add_week():
            ...
            play['year'] = (datetime.strptime(row['game_date'], '%Y-%m-%d')).year
            play['month'] = (datetime.strptime(row['game_date'], '%Y-%m-%d')).month
            play['day'] = (datetime.strptime(row['game_date'], '%Y-%m-%d')).day
            if play['day'] < 10:
            ...
            play['week'] = get_nfl_week(row['game_date'])
...
def load_big(year=2012, small=True):
    if year == 2023:
        ...
        # df1 = pickle_wrap('pbp_data\\pkl\\2019_pbp.pkl', lambda: pd.read_excel('/Users/mohan_007/Downloads/pbp2023_csv.xlsx'))
        # df1 = pickle_wrap('pbp_data\\pkl\\2019_pbp.pkl', lambda: pd.read_csv('/Users/mohan_007/Downloads/pbp_csv.csv'))
        # df1 = pickle_wrap('pbp_data\\pkl\\2019_pbp.pkl', lambda: pd.read_csv('2023_2024.csv'))
        from .models import play_by_play
        df1 = pd.DataFrame.from_records(list(play_by_play.objects.values()))

        #/Users/mohan_007/Documents/play_by_play_2024.csv
...
def make_weeks_from_df(year,plays_df):
...
def main_1():
    ...
    YEAR = 2023
    SMALL = False
    df = load_big(year=YEAR, small=SMALL)
    plays_df =  iterate_df(df)
    print('go:', plays_df['week'].unique())
    return make_weeks_from_df(YEAR,plays_df)
