...
class Game():
    def __init__(self, home_team, away_team, df, co_eff,is_custom_sim,print=True, max_effective_time=900):
        self.home = home_team
        self.away = away_team
        if random() > 0.5:
        ...
        self.co_eff = co_eff 
        self.is_custom_sim = is_custom_sim
        ...
        self.down = None
        self.print = print
        self.basic_predictors = {'run_vs_pass': Basic_Predictor(df, 'run_vs_pass ~ pos_team + pos_winning*time_left*goal_yd + pos_minus_def_score*time_left + ydstogo',self.is_custom_sim,self.co_eff['run_vs_pass'],
                                                                self.pos_team,self.def_team,logistic=True),
                                 'punt': Basic_Predictor(df[df['down'] == 4],
                                                         'punt ~ pos_winning*ydstogo*goal_yd*time_left*pos_minus_def_score + pos_team',self.is_custom_sim,self.co_eff['punt'],
                                                         self.pos_team,self.def_team,logistic=True),
                                 'punt_yds': Basic_Predictor(df[df['punt'] == 1], 'punt_yds ~ goal_yd',self.is_custom_sim,self.co_eff['punt_yds'],self.pos_team,self.def_team,), # maybe wanna make this a DM/PM
                                 'FG_attempted': Basic_Predictor(df[(df['down'] == 4) & (df['punt'] == 0)],
                                                                ...
                                                                ,self.is_custom_sim ,self.co_eff['FG_attempted'] ,self.pos_team,self.def_team,logistic=True),
                                 'FG_made': Basic_Predictor(df[df['FG_attempted'] == 1],
                                                                 'FG_made ~ goal_yd', # had to take out + pos_team for the low weeks
                                                                self.is_custom_sim,self.co_eff['FG_made'] ,self.pos_team,self.def_team, logistic=True),
                                 'fumble': Basic_Predictor(df, 'fumble ~ def_team + pos_team + time_left + run_vs_pass',self.is_custom_sim,self.co_eff['fumble'],self.pos_team,self.def_team, logistic=True),
                                 'fumble_lost': Basic_Predictor(df[df['fumble'] == 1], 'fumble_lost ~ run_vs_pass',self.is_custom_sim, self.co_eff['fumble_lost'],self.pos_team,self.def_team,logistic=True),
                                 'fumble_yds': Basic_Predictor(df[df['fumble'] == 1], 'fumble_spot_yds ~ def_team',self.is_custom_sim,self.co_eff['fumble_yds'],self.pos_team,self.def_team),
                                 'interception': Basic_Predictor(df[df['run_vs_pass'] == 0], 'interception ~ time_left + pos_team + def_team',self.is_custom_sim,self.co_eff['interception'],self.pos_team,self.def_team, logistic=True),
                                 'int_yds': Basic_Predictor(df[df['interception'] == 1], 'int_spot_yds ~ def_team',self.is_custom_sim,self.co_eff['int_yds'],self.pos_team,self.def_team)}
